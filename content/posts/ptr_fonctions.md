...
lastmod: 2021-01-24T22:16:49+01:00
draft: false
author: "Oxygel.fr"
...
Ces données peuvent très bien être constantes et codées en dur dans le programme, soit des paramètres que l'on passe à la fonction.
...
Des vérifications supplémentaires doivent être effectuées selon le véhicule, ce qui paraît beaucoup plus réaliste, on ne prend pas les même précautions entre une voiture et simplement marcher dans la rue *(c'est vrai que la vitesse joue un peu)*
...
Cette solution semble satifaisante pour le moment. Cependant on se rend compte qu'elle n'est viable que pour un nombre fini de véhicule mais aussi et surtout **court**. 
Au niveau de la maintenance c'est un désastre, chaque fois que l'on ajoute une variante de `Locomotion` il faut encore agrandir cette fonction, on sent plutot qu'on a caché le problème sous le tapis.
...
Nous allons donc séparer le code précédent de l'implémentation des moyens de locomotions.
Ainsi on a
